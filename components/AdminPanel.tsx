...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  // ========== BANNER MANAGEMENT ==========
  const handleAddBanner = async (movieId: string, customUrl?: string) => {
    const movie = movieList.find(m => m.id === movieId);
    if (!movie) return;

    setLoading(true);
    try {
      // Custom URL > detailBanner (TMDB backdrop) > thumbnail
      const bannerImage = customUrl || movie.detailBanner || movie.thumbnail;
  ...
  const handleUpdateBannerImage = async (bannerId: string, newImageUrl: string) => {
    ...
    try {
      ...
      showSuccess('✅ Banner image updated!');
      fetchBanners();
    } catch (err: any) {
      alert('❌ Error: ' + err.message);
    }
          ...
          {activeTab === 'banners' && (
            <div className="space-y-6">
              ...
              <div>
                <h3 className="text-lg font-bold text-white mb-4">
                  ...
                  <span className="text-sm text-gray-500 font-normal ml-2">
                    এইগুলো home page এ বড় banner হিসেবে দেখাবে
                  </span>
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {banners.map(banner => {
                    const movie = movieList.find(m => m.id === banner.movieId);
                    const [isEditingImg, setIsEditingImg] = React.useState(false);
                    const [newImageUrl, setNewImageUrl] = React.useState(banner.image);
                    ...
                    return (
                      <div key={banner.id} className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700">
                        ...
                        <div className="p-4 space-y-3">
                          ...
                          {isEditingImg && (
                            <div className="bg-gray-900/50 border border-blue-500/30 rounded-lg p-3 space-y-2">
                              <label className="text-xs text-blue-300 font-semibold block">
                              ...
                              <div className="flex gap-2">
                                <button
                                  onClick={() => {
                                    handleUpdateBannerImage(banner.id, newImageUrl);
                                    setIsEditingImg(false);
                                  }}
                                  disabled={loading || newImageUrl === banner.image}
              ...
              <div>
                ...
                <div className="bg-blue-900/20 border border-blue-500/30 rounded-xl p-4 mb-4 space-y-2">
                  <p className="text-blue-300 text-sm font-bold">🖼️ Custom Banner Image (Optional)</p>
                  <p className="text-gray-500 text-xs">এই URL দিলে movie এর thumbnail এর বদলে এটা banner এ দেখাবে। খালি রাখলে TMDB backdrop বা thumbnail use হবে।</p>
                  <input
                    type="text"
                    value={customBannerUrl}
                    onChange={(e) => setCustomBannerUrl(e.target.value)}
                    placeholder="Custom Banner URL (16:9 landscape, e.g. 1280×720) — Optional"
                    className="w-full px-4 py-2.5 bg-gray-900 border border-blue-500/20 rounded-lg text-white text-sm"
                  />
                  {customBannerUrl && (
                    <div className="relative">
                      <img src={customBannerUrl} alt="Banner Preview" className="w-full h-36 object-cover rounded-lg border border-blue-500/20" 
                        onError={(e) => { e.currentTarget.style.display='none'; }}
                      />
                      <div className="absolute top-2 right-2 bg-black/60 text-white text-[9px] px-2 py-0.5 rounded-full">Custom Banner Preview</div>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {movieList.map(movie => (
                    <div key={movie.id} className="bg-gray-800 rounded-lg p-3 flex items-center gap-3">
                      <img src={movie.detailBanner || movie.thumbnail} alt={movie.title} className="w-20 h-12 object-cover rounded" />
                      <div className="flex-1 min-w-0">
                        <h4 className="text-white font-medium truncate">{movie.title}</h4>
                        <p className="text-gray-400 text-xs">{movie.category}</p>
                        {movie.detailBanner && <p className="text-green-400 text-[10px] mt-0.5">✓ TMDB Backdrop আছে</p>}
                      </div>
                      <button
                        onClick={() => handleAddBanner(movie.id, customBannerUrl || undefined)}
                        disabled={loading}
